...
int main()
{
    ...
    AssetMgr->GetAsset<TestFile>(AssetID{ 123 }).Render();
    AssetMgr->GetAsset<TestFile>("Hello/Hello.txt").Render();
    std::cout << AssetMgr->IsAssetLoadedForType<TestFile>(AssetID{ 123 });
    std::cout << AssetMgr->IsAssetLoadedForType<TestFile>(AssetID{ 789 });
    std::cout << AssetMgr->IsAssetLoadedForType<TestFile>("Hello/Hello.txt");
    std::cout << AssetMgr->IsAssetLoadedForType<TestFile>("Hello/Hello123.txt");
    std::cout << AssetMgr->IsAssetLoaded(AssetID{ 123 });
    std::cout << AssetMgr->IsAssetLoaded(AssetID{ 789 });
    std::cout << AssetMgr->IsAssetLoaded("Hello/Hello.txt");
    std::cout << AssetMgr->IsAssetLoaded("Hello/Hello123.txt") << std::endl;
   ...
   //std::cout << Vec3f(1,1, 0) + Mat3f{}.SetRotation(Quatf({ 1,1,1 }, 60.0f)) * Vec3f(3,0,0) << std::endl;
    ...
    rttr::type t = rttr::type::get_by_name(typeid(BE::Transform).name());
    
    rttr::instance inst = te2;
    for (auto prop:t.get_properties())
    {
        std::cout << prop.get_name() << " of type " << prop.get_type().get_name();
        rttr::variant variant = prop.get_value(inst);
        if (variant.is_type<Vec3<float>>())
        {
            const Vec3<float>& vec = variant.get_value<Vec3<float>>();
            std::cout << vec;
        }
        else if (variant.is_type<BE::Quatf>())
        {
            const BE::Quatf& quat = variant.get_value<BE::Quatf>();
            std::cout << quat;
        }
        else
        {
