...
namespace BE
{
	...
	{
		//Get the angle first
		T angle = this->GetRotation(false);
	...
	Vec3<T> Quaternion<T>::GetEulerAngles() const
	{
		...
		if (Math::Abs<T>(sp) >= 1)
			result.x = std::copysign(static_cast<T>(90), sp);		//Gimbal lock: Cap at 90 degrees
		else
			result.x = Math::ASinDeg<T>(sp);

		//Yaw - Y
		T sycp = 2 * (r * j + k * i);
		T cycp = 1 - 2 * (i * i + j * j);
		result.y = Math::ATanDeg(sycp, cycp);
		
		//Roll - Z
		T srcp = 2 * (r * k + i * j);
		T crcp = 1 - 2 * (k * k + i * i);
		result.z = Math::ATanDeg(srcp, crcp);
