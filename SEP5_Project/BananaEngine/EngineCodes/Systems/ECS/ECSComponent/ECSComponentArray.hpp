...
namespace BE
{
	...
	void ECSCompArray<ComponentType>::RemoveData(EntityID _eid)
	{
		...
		size_t indexToReplace = m_IDToIndexMap[_eid];
		...
		m_componentData[indexToLast].~ComponentType();
	...
	template <typename ComponentType>
	void ECSCompArray<ComponentType>::InsertDataNT(EntityID _eid, IECSComponent&& _data)
	{
		this->InsertData(_eid, dynamic_cast<ComponentType&&>(_data));
	}
