...
namespace BE
{
	...
	void ECSComponentMgr::AddComponentNT(const std::string& _component, EntityID _eid, IECSComponent&& _data)
	{
		m_ComponentData.at(m_compName_to_compID.at(_component))->InsertDataNT(_eid, std::move(_data));
	}
	...
	ComponentID ECSComponentMgr::GetComponentID(const std::string& _componentName)
	{
